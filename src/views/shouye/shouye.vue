<template>
  <div class="flex-col page">
    ...
    <div class="flex-col story_list">
      ...
      <div :class="`flex-col section_9 ${sectionActive && 'section_active'}`" @click="setSectionActive">
        <div class="flex-col items-center text-wrapper_2"><span class="text_9">第三章· 碰撞</span></div>
        <div :class="`cloes_active_icon ${sectionActive && 'icon_active'}`" @click="setSectionHidden">x</div>
        <span :class="`text_10  ${sectionActive && 'section_hidden'}`">
        ...
        <div :class="`justify-between story_detail ${sectionActive && 'story_detail_active'}`">
          <div class="flex-col desc">
            ...
            <span>
              如果数据分析得无误，这颗不明行星将会在218.5天后，与元星球相撞，博士连续3天无休止地重复推演计算
            </span>
            <span>
              但得到的结果与时间都是一样，于是，博士连夜会见了国家首脑，并在第二天紧急召开了北部联合国最高会议，商讨应对即将到来的元末日，至此，元计划应运而生，但北部对于南部的认知仅限于超重物质的存在，所以这次对南部的大规模探索，充满着未知的风险，但他们别无选择，因为博士深刻地明白......
            </span>
        ...
        <div :class="`section-more-icon  ${sectionActive && 'section_hidden'}`" />
      </div>
    </div>
    <WebFooter />
  </div>
...
export default {
  ...
  methods: {
    setSectionActive () {
      if (!this.sectionActive) this.sectionActive = true
    },
    setSectionHidden (e) {
      e.stopPropagation()
